...
select c.id, c.first_name, c.last_name, c.email, a.account_no, a.balance from clients as c left join accounts as a on c.id = a.client_id;

select a.account_no, a.client_id, b.name from accounts as a left join account_types as b on a.type = b.id;

select a.id, a.amount, b.name from transactions as a left join transactions_types as b on a.transaction_type = b.id;

select a.id, a.amount, b.name from transactions as a right join transactions_types as b on a.transaction_type = b.id;


create view v_clients_accounts as select c.id, c.first_name, c.last_name, c.email, a.account_no, a.balance from clients as c left join accounts as a on c.id = a.client_id;

create view v_accounts_types as select a.account_no, a.client_id, b.name from accounts as a left join account_types as b on a.type = b.id;

create view v_transactions_detail as select a.id, a.amount, b.name from transactions as a left join transactions_types as b on a.transaction_type = b.id;

create view v_transations_types_detail as select a.id, a.amount, b.name from transactions as a right join transactions_types as b on a.transaction_type = b.id;